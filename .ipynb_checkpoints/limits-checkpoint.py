...
#             self.ax.set_ylabel(r'$E\Phi$ [cm$^{-2}$ s$^{-1}$ sr$^{-1}$]')
    ...
    #JCF: expanding this regression to cover full energy spectrum 11/28/2022
    def _get_i3_mu_range(cls):
        energy = np.arange(1e5, 1e11, 1e5)
        upper = np.maximum(cls._i3_nu_fit(energy, offset=0.9, slope=-2.),
                           cls._i3_nu_fit(energy, offset=1.2, slope=-2.13))
        lower = np.minimum(cls._i3_nu_fit(energy, offset=0.9, slope=-2.26),
                           cls._i3_nu_fit(energy, offset=0.63, slope=-2.13))
...
#             self.neutrino_models.append(prot10)
            ...
            # arrow_SFRHiE = self.ax.annotate('', xy=(energy2[33],fluxInterp_SFRHiE[33]), xytext=(energy2[33],flux_kot[33]),                 arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))
            # arrow_SFRHiE = self.ax.annotate('', xy=(energy2[32],fluxInterp_SFRHiE[32]), xytext=(energy2[32],flux_kot[32]),                 arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))
            # arrow_SFRHiE = self.ax.annotate('', xy=(7e7,3e-17), xytext=(7e7,1.1e-16),                 arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))            
            # dummyPlot = self.ax.plot(energy2, fluxInterp_SFRHiE, color='red', linestyle="--")
            # dummyPlot = self.ax.plot(energy2, flux_kot, color='red', linestyle="--")
            ...
            # arrow_heavyComp = self.ax.annotate('', xy=(energy2[45],fluxInterp_heavyComp[45]), xytext=(energy2[45],fluxInterp[45]), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))
            # arrow_heavyComp = self.ax.annotate('', xy=(energy2[39],fluxInterp_heavyComp[39]), xytext=(energy2[39],fluxInterp[39]), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))
            # dummyPlot = self.ax.plot(energy2, fluxInterp, color='blue', linestyle="--")
            # dummyPlot = self.ax.plot(energy2, fluxInterp_heavyComp,color='blue', linestyle="--")
...
#                 self.neutrino_models.append(prot10)
                ...
                # arrow_heavyComp = self.ax.annotate('', xy=(energy[37]+3e9,fluxInterp_lowE[37]), xytext=(2.5e10,fluxInterp_lowE[37]), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))               
                # arrow_heavyComp = self.ax.annotate('', xy=(energy[36]+3e9,fluxInterp_lowE[36]), xytext=(2.5e10,fluxInterp_lowE[36]), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))
                # arrow_heavyComp = self.ax.annotate('', xy=(energy[36]+3e9,1.2e-18), xytext=(2.5e10,1.2e-18), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))              
                # arrow_heavyComp = self.ax.annotate('', xy=(energy[36]+3e9,2e-18), xytext=(2.5e10,2e-18), arrowprops=dict(arrowstyle='<|-|>',color='rosybrown',lw=2))                    
                # dummyPlot = self.ax.plot(energy, fluxInterp_lowE, color='green', linestyle="--", lw=3)
...
#                                        label=r'$E^2\frac{dN}{dE}=10^{-9}$ [GeV$\,cm^{-2}\, s^{-1} \,sr^{-1}]$')
                ...
                self.ax.annotate("$E^2dN/dE=10^{-9}$ [GeV$\,cm^{-2}\, s^{-1} \,sr^{-1}]$",(1e6,1e-17),rotation=-43, color="gray")
                self.neutrino_models.append(prot10)
...
#             self.neutrino_models.append(wind)
    ...
    def add_experiment(self, name):
        ...
        if self.e_power==1:
            self.ax.annotate('IceCube \n (9yr limit)',
                             xy=(1.3e7, 1.4e-15), xycoords='data',
                             horizontalalignment='center', color='dodgerblue', rotation=-30)
        ...
        if name=='grand_10k':
            ...
            self.ax.plot(energy, flux,
                         color='dodgerblue')
            #JCF:  This seems to be the IceCube band that I want to extrapolate out.
            ...
            self.ax.fill_between(energy, band_min, band_max,
                                 edgecolor='dodgerblue', facecolor='azure', alpha=0.4)
            ...
            if self.e_power==1:
                ...
                #                  horizontalalignment='left', color='darkorange')
                self.ax.annotate('ANITA 1-4',
                                 xy=(2e9, 3e-15*self.e_bins), xycoords='data',
                                 horizontalalignment='left', color='darkorange')                
...
#                          color='pink')
            ...
            if self.e_power==1:
                self.ax.annotate('Auger (14.7yr)',
                                 xy=(6.5e7,2e-16*self.e_bins), xycoords='data',
                                 horizontalalignment='left', color='forestgreen', rotation=-58)
...
#                                  horizontalalignment='left', color='#2288AA', rotation=-50)
                self.ax.annotate('ARA (2x4yr)',
                                 xy=(3.2e8, 5.2e-16*self.e_bins), xycoords='data',
                                 horizontalalignment='left', color='#2288AA', rotation=-50)
...
#                                  horizontalalignment='left', color='black', rotation=-47)
            ...
            if self.e_power==1:
                ...
                self.ax.annotate('ARIANNA (4.5yr)',
                                 xy=(5e9, 1.5e-16*self.e_bins), xycoords='data',
                                 horizontalalignment='left', color='firebrick', rotation=-28)
    ...
    def add_limit(self, name, energies, veffs, stations=1, years=1, color=None, linestyle=None, label=None, linewidth=3):
...
# ara_available = ((851+365)*ara_100m + (3351+365*(2+7/12+10/12))*ara_200m + (306*(1+10/12))*ara_200mpa)/365.25  #2012 - 2019
ara_available = ((851+365+366+2*365)*ara_100m + (351+365*(2+7/12+10/12)+2*366+121+15)*ara_200m + (306*(1+10/12)+366+365*2)*ara_200mpa)/365.25  #2012 - 2022
ara_yearly = ara_100m + 3*ara_200m + ara_200mpa
# ara_projected = ara_available + 3*ara_yearly #Previous Ara Grant
# ara_projected = ara_available + 11*ara_yearly #2019 to #2030
ara_projected_2025 = ara_available + 3*ara_yearly #2022 to #2025
ara_projected_2026 = ara_available + 4*ara_yearly #2022 to #2026
ara_projected_2030 = ara_available + 8*ara_yearly #2022 to #2030
